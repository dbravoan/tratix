...
@section('title', 'Nuevo contrato - Asistente paso a paso')
...
<div class="max-w-7xl mx-auto px-2 sm:px-4 py-4" x-data="contractWizard()">
    ...
    {{-- Main Form Container with Side-by-Side Live Preview Layout on large screens --}}
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {{-- Left / Main Column: Walkthrough Form --}}
        <div class="lg:col-span-7 xl:col-span-7 space-y-6">
            <form method="POST" action="{{ route('contracts.store') }}" id="contract-form">
                ...
                <div x-show="currentStep === 1" x-transition.opacity.duration.250ms class="space-y-6">
                    <section class="bg-slate-800/95 border border-slate-700 rounded-xl p-5 shadow-sm">
                        <div class="flex items-center justify-between border-b border-slate-700 pb-3 mb-4">
                            <div>
                                <h2 class="font-bold text-slate-100 text-base">⚡ Plantillas recomendadas</h2>
                                <p class="text-xs text-slate-400">Selecciona una plantilla para rellenar automáticamente la base legal del contrato.</p>
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                            @foreach([
                                ['arras', 'Contrato de arras penitenciales', 'Reserva de inmueble con señal (art. 1454 CC)'],
                                ['inmuebles', 'Compraventa de inmueble', 'Vivienda, local o terreno'],
                                ['alquiler', 'Alquiler de vivienda', 'Arrendamiento de vivienda habitual o temporada'],
                                ['vehiculos', 'Compraventa de vehículo', 'Coche, moto o furgoneta usada'],
                                ['bienes_muebles', 'Bienes muebles', 'Maquinaria, electrónica, mobiliario'],
                                ['servicios', 'Prestación de servicios', 'Servicios profesionales y freelance'],
                                ['prestamo', 'Préstamo entre particulares', 'Préstamo de dinero sin intereses/con interés'],
                                ['nda', 'Acuerdo de confidencialidad (NDA)', 'Protección de secretos y know-how'],
                            ] as [$type, $label, $desc])
                                <button type="button" @click="applyTemplate('{{ $type }}', '{{ $label }}', '{{ $desc }}')"
                                    class="text-left p-3 rounded-lg border border-slate-700 bg-slate-900/60 hover:bg-slate-700/70 hover:border-emerald-500/80 transition group flex flex-col justify-between">
                                    <span class="font-semibold text-xs text-slate-200 group-hover:text-emerald-300">{{ $label }}</span>
                                    <span class="text-[11px] text-slate-400 mt-1">{{ $desc }}</span>
                                </button>
                            @endforeach
                        </div>
                    </section>
                ...
                {{-- PASO 2: Mis Datos de Identificación (Tu Parte) --}}
                    ...
                    <section class="bg-slate-800/95 border border-slate-700 rounded-xl p-5 shadow-sm">
                        <div class="flex items-center justify-between border-b border-slate-700 pb-3 mb-4">
                            <div>
                                ...
                                <p class="text-xs text-slate-400">Verifica tus datos fiscales por defecto o escanea tu DNI para autocompletar.</p>
                            </div>
                        </div>
                        <div x-show="form.creator_role === 'vendedor'">
                            @include('contracts._party_form', ['role' => 'seller', 'label' => 'Vendedor'])
                        </div>
                        <div x-show="form.creator_role === 'comprador'">
                            @include('contracts._party_form', ['role' => 'buyer', 'label' => 'Comprador'])
                        </div>
                    </section>
                </div>

                {{-- PASO 3: Datos de la Otra Parte (Contraparte) --}}
                    ...
                    <section class="bg-slate-800/95 border border-slate-700 rounded-xl p-5 shadow-sm space-y-4">
                        <div class="flex items-center justify-between border-b border-slate-700 pb-3">
                            <div>
                                ...
                                <p class="text-xs text-slate-400">¿Dispones ya de los datos fiscales completos de la otra parte o prefieres invitarle a completarlos?</p>
                            </div>
                        </div>

                        {{-- Mode Selector Cards --}}
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <label class="p-4 rounded-xl border cursor-pointer transition flex items-start gap-3"
                                   :class="delegateCounterparty ? 'bg-emerald-950/60 border-emerald-500 text-slate-100 shadow-md' : 'bg-slate-900/60 border-slate-700 text-slate-400 hover:border-slate-600'">
                                <input type="radio" name="delegation_toggle" :value="true" x-model="delegateCounterparty" class="mt-1 text-emerald-500 focus:ring-emerald-500">
                                <div>
                                    <span class="font-bold text-xs block text-emerald-300">📩 Invitar a la otra parte a rellenar sus datos (Recomendado)</span>
                                    <span class="text-[11px] text-slate-300 block mt-1">Solo necesitas su correo o WhatsApp. La otra parte podrá escanear su DNI y añadir su domicilio en la revisión antes de firmar.</span>
                                </div>
                            </label>

                            <label class="p-4 rounded-xl border cursor-pointer transition flex items-start gap-3"
                                   :class="!delegateCounterparty ? 'bg-emerald-950/60 border-emerald-500 text-slate-100 shadow-md' : 'bg-slate-900/60 border-slate-700 text-slate-400 hover:border-slate-600'">
                                <input type="radio" name="delegation_toggle" :value="false" x-model="delegateCounterparty" class="mt-1 text-emerald-500 focus:ring-emerald-500">
                                <div>
                                    <span class="font-bold text-xs block text-slate-200">📝 Rellenar todos los datos de la otra parte ahora</span>
                                    <span class="text-[11px] text-slate-400 block mt-1">Si ya tienes su NIF, domicilio completo o fotos de su DNI.</span>
                                </div>
                            </label>
                        </div>
                        <input type="hidden" name="invite_counterparty_to_fill" :value="delegateCounterparty ? '1' : '0'">

                        {{-- Delegated Simple Form --}}
                        <div x-show="delegateCounterparty" class="p-4 bg-slate-900/80 border border-slate-700 rounded-xl space-y-3">
                            <div class="flex items-center gap-2 text-xs text-emerald-300">
                                <span>💡</span>
                                <span>Al crear el contrato, se generará un enlace directo de revisión y firma que podrás compartir por email o WhatsApp.</span>
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                <div>
                                    <label class="block text-xs font-semibold text-slate-200 mb-1">Nombre orientativo (opcional)</label>
                                    <input :name="`${form.creator_role === 'vendedor' ? 'buyer' : 'seller'}[full_name]`"
                                           :id="`${form.creator_role === 'vendedor' ? 'buyer' : 'seller'}_input_full_name`"
                                           class="w-full border border-slate-600 bg-slate-950 text-slate-100 rounded-md px-3 py-2 text-xs focus:ring-2 focus:ring-emerald-500 placeholder-slate-500"
                                           placeholder="p. ej. Carlos Comprador">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-slate-200 mb-1">Email de la otra parte</label>
                                    <input type="email"
                                           :name="`${form.creator_role === 'vendedor' ? 'buyer' : 'seller'}[email]`"
                                           :id="`${form.creator_role === 'vendedor' ? 'buyer' : 'seller'}_email`"
                                           class="w-full border border-slate-600 bg-slate-950 text-slate-100 rounded-md px-3 py-2 text-xs focus:ring-2 focus:ring-emerald-500 placeholder-slate-500"
                                           placeholder="contacto@otraparte.com">
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-slate-200 mb-1">Teléfono / WhatsApp</label>
                                    <input :name="`${form.creator_role === 'vendedor' ? 'buyer' : 'seller'}[phone]`"
                                           :id="`${form.creator_role === 'vendedor' ? 'buyer' : 'seller'}_phone`"
                                           class="w-full border border-slate-600 bg-slate-950 text-slate-100 rounded-md px-3 py-2 text-xs focus:ring-2 focus:ring-emerald-500 placeholder-slate-500"
                        ...
                        {{-- Full Party Form --}}
                        <div x-show="!delegateCounterparty">
                            <div x-show="form.creator_role === 'vendedor'">
                                @include('contracts._party_form', ['role' => 'buyer', 'label' => 'Comprador'])
                            </div>
                            <div x-show="form.creator_role === 'comprador'">
                ...
                {{-- PASO 5: Revisión Final y Creación --}}
                <div x-show="currentStep === 5" x-transition.opacity.duration.250ms class="space-y-6">
                    <section class="bg-slate-800/95 border border-slate-700 rounded-xl p-5 shadow-sm space-y-4">
                        <h2 class="font-bold text-emerald-400 text-base border-b border-slate-700 pb-2">5. Resumen previo a la creación del contrato</h2>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                            ...
                            <div class="bg-slate-900/80 p-3.5 rounded-lg border border-slate-700">
                                <div class="flex items-center justify-between mb-1">
                                    ...
                                    <span x-show="form.creator_role === 'vendedor'" class="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-950 text-emerald-300 border border-emerald-800">Mi Parte</span>
                                    <span x-show="form.creator_role !== 'vendedor' && delegateCounterparty" class="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-950 text-amber-300 border border-amber-800">Completará en revisión</span>
                                </div>
                            ...
                            <div class="bg-slate-900/80 p-3.5 rounded-lg border border-slate-700">
                                <div class="flex items-center justify-between mb-1">
                                    ...
                                    <span x-show="form.creator_role === 'comprador'" class="text-[10px] font-bold px-2 py-0.5 rounded bg-emerald-950 text-emerald-300 border border-emerald-800">Mi Parte</span>
                                    <span x-show="form.creator_role !== 'comprador' && delegateCounterparty" class="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-950 text-amber-300 border border-amber-800">Completará en revisión</span>
                                </div>
                        ...
                        <div class="p-3.5 bg-emerald-950/40 border border-emerald-800 rounded-lg text-xs text-emerald-300 flex items-start gap-2.5">
                            <span class="text-lg shrink-0">🛡️</span>
                            <div>
                                <p class="font-semibold">Listo para generar el documento legal y comenzar la revisión</p>
                                <p class="text-slate-300 text-[11px] mt-0.5">Al crear el contrato se generarán las cláusulas adaptadas a la legislación vigente, se adjuntarán los DNI escaneados y se habilitará el enlace de firma electrónica y evidencias.</p>
                            </div>
                        </div>
                    </section>
                </div>
                ...
                <div class="flex items-center justify-between pt-4 border-t border-slate-800">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <button type="submit" x-show="currentStep === 5" class="btn-primary px-8 py-3 text-sm font-bold flex items-center gap-2 shadow-lg hover:shadow-emerald-500/20">
                            <span>🚀 Crear y Guardar Contrato</span>
                        </button>
                    </div>
                </div>
            </form>
        ...
        <div class="lg:col-span-5 xl:col-span-5 sticky top-6 space-y-4" x-show="showPreviewOnDesktop || isPreviewModalOpen" :class="{'fixed inset-0 z-50 bg-slate-950/90 backdrop-blur-md p-4 flex items-center justify-center': isPreviewModalOpen}">
            <div class="bg-slate-900 border border-slate-700/90 rounded-2xl p-5 shadow-2xl flex flex-col h-[750px] max-h-[85vh] w-full" :class="{'max-w-3xl': isPreviewModalOpen}">
                ...
                <div class="flex items-center justify-between border-b border-slate-800 pb-3 mb-3 shrink-0">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p class="text-[11px] text-slate-400">Las variables en verde se actualizan en tiempo real al escribir.</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                ...
                {{-- Preview Body (Document text with variables) --}}
                <div class="overflow-y-auto pr-2 space-y-4 font-serif text-xs leading-relaxed text-slate-300 bg-slate-950/70 p-4 rounded-xl border border-slate-800/80 select-text" id="live_contract_preview">
                    ...
                    <div class="text-center pb-2 border-b border-slate-800">
                        ...
                        <p class="text-[11px] font-sans text-slate-400 mt-1">
                            En <span class="preview-var text-emerald-400 font-semibold" id="preview_city">[CIUDAD DE FIRMA]</span>, a <span class="preview-var text-emerald-400 font-semibold" id="preview_signing_date">[FECHA DE FIRMA]</span>
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mb-2">
                            ...
                            <span class="preview-var text-emerald-400 font-semibold" id="preview_seller_name">[NOMBRE / RAZÓN SOCIAL VENDEDOR]</span>,
                            con documento fiscal / NIF <span class="preview-var text-emerald-400 font-semibold" id="preview_seller_tax_id">[NIF/CIF VENDEDOR]</span>,
                            y domicilio en <span class="preview-var text-emerald-400 font-semibold" id="preview_seller_address">[DOMICILIO VENDEDOR]</span>.
                        </p>
                        <p>
                            <strong>DE OTRA PARTE</strong>, como PARTE COMPRADORA:
                            <span class="preview-var text-emerald-400 font-semibold" id="preview_buyer_name">[NOMBRE / RAZÓN SOCIAL COMPRADOR]</span>,
                            con documento fiscal / NIF <span class="preview-var text-emerald-400 font-semibold" id="preview_buyer_tax_id">[NIF/CIF COMPRADOR]</span>,
                            y domicilio en <span class="preview-var text-emerald-400 font-semibold" id="preview_buyer_address">[DOMICILIO COMPRADOR]</span>.
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p>
                            ...
                            <span class="preview-var text-emerald-400 font-semibold" id="preview_object_description">[DESCRIPCIÓN DEL BIEN O INMUEBLE]</span>.
                    ...
                    <div>
                        ...
                        {{-- Cláusula 1: Objeto --}}
                        <p class="mb-2">
                            <strong>PRIMERA. Objeto.</strong> La parte vendedora transmite a la parte compradora el bien descrito (<span class="preview-var text-emerald-400 font-semibold" id="preview_object_type">[TIPO DE BIEN]</span>), que adquiere en las condiciones pactadas.
                        </p>

                        {{-- Cláusula 2: Precio --}}
                        <p class="mb-2">
                            <strong>SEGUNDA. Precio y condiciones de pago.</strong> El precio total de la presente operación se fija en la cantidad de <span class="preview-var text-emerald-400 font-semibold font-sans font-bold" id="preview_price">[0,00 EUR]</span>.
                            <span class="preview-var text-emerald-400" id="preview_payment_terms">[CONDICIONES DE PAGO / ARRAS]</span>
                        </p>

                        {{-- Cláusula 3: Específica según tipo --}}
                        <div class="mb-2 p-2.5 rounded bg-emerald-950/30 border border-emerald-800/60" id="preview_specific_clause">
                            <strong class="text-emerald-300 font-sans block mb-1" id="preview_specific_title">TERCERA. Naturaleza de las arras y régimen penitencial (Art. 1454 Código Civil)</strong>
                            <span class="text-slate-300" id="preview_specific_body">La cantidad entregada tiene carácter de arras penitenciales. Si el comprador desiste, las perderá; si desiste el vendedor, las devolverá duplicadas.</span>
                        </div>

                        {{-- Cláusula 4: Plazo y Entrega --}}
                        <p class="mb-2">
                            <strong>CUARTA. Plazo de formalización y entrega.</strong> <span class="preview-var text-emerald-400" id="preview_delivery_terms">[PLAZOS Y ELEVACIÓN A PÚBLICO]</span>
                        </p>
                    ...
                    <div class="pt-3 border-t border-slate-800 grid grid-cols-2 gap-4 text-center font-sans text-[11px]">
                        <div class="p-2 border border-dashed border-slate-700 rounded bg-slate-900/60">
                            ...
                            <span class="font-bold text-slate-200" id="preview_signature_seller">[VENDEDOR]</span>
                        </div>
                        <div class="p-2 border border-dashed border-slate-700 rounded bg-slate-900/60">
                            <span class="text-slate-400 block mb-1">Por la Parte Compradora</span>
                            <span class="font-bold text-slate-200" id="preview_signature_buyer">[COMPRADOR]</span>
                        </div>
                    </div>
                </div>
            </div>
...
{{-- Include Tesseract.js for high-accuracy client-side OCR --}}
...
<script>
    function contractWizard() {
        return {
            ...
            delegateCounterparty: {{ old('invite_counterparty_to_fill', '1') == '1' ? 'true' : 'false' }},
            form: {
                contract_type: '{{ old('contract_type', 'arras') }}',
                title: '{{ old('title', 'Contrato de arras penitenciales') }}',
                object_type: '{{ old('object_type', 'Vivienda') }}',
                object_description: @json(old('object_description', '')),
                quantity: '{{ old('quantity', 1) }}',
                city: '{{ old('city', 'Madrid') }}',
                signing_date: '{{ old('signing_date', now()->format('Y-m-d')) }}',
                effective_date: '{{ old('effective_date', '') }}',
                creator_role: '{{ old('creator_role', 'vendedor') }}',
                price_amount: '{{ old('price_amount', '') }}',
                tax_amount: '{{ old('tax_amount', 0) }}',
                currency: '{{ old('currency', 'EUR') }}',
                payment_terms: @json(old('payment_terms', '')),
                delivery_terms: @json(old('delivery_terms', '')),
                warranties: @json(old('warranties', '')),
            },
            typeHints: {
                arras: {
            ...
            get steps() {
                ...
                return [
                    { shortName: '1. Objeto & Rol', name: 'Tipo y Objeto', title: '1. Elige el tipo de contrato, tu posición y el objeto', description: 'Define qué vas a acordar, tu posición en la operación, la ciudad de firma y las fechas.' },
                    { shortName: '2. Mis Datos', name: 'Tus Datos', title: '2. Tus datos de identificación (' + (isSeller ? this.currentHints.sellerRoleLabel : this.currentHints.buyerRoleLabel) + ')', description: 'Verifica tus datos legales y fiscales para formalizar el acuerdo.' },
                    { shortName: '3. Contraparte', name: 'La Otra Parte', title: '3. Identificación de la otra parte (' + (isSeller ? this.currentHints.buyerRoleLabel : this.currentHints.sellerRoleLabel) + ')', description: 'Invita a la otra parte a rellenar sus datos o complétalos si ya dispones de ellos.' },
                    { shortName: '4. Condiciones', name: 'Condiciones y Pago', title: '4. Condiciones económicas y cláusulas', description: 'Indica el precio, condiciones de pago, entrega y garantías aplicables.' },
                    { shortName: '5. Revisar', name: 'Revisión Final', title: '5. Resumen previo a la creación oficial', description: 'Comprueba el borrador final antes de generar el contrato oficial.' },
                ];
            },
            init() {
                this.$nextTick(() => {
            ...
            updatePreview() {
                ...
                let filled = 0;
                const check = (val) => { if (val && val.trim()) filled++; };

                // Title & City
                el('preview_title', this.form.title || 'CONTRATO DE COMPRAVENTA', !this.form.title);
                ...
                // Object
                el('preview_object_type', this.form.object_type || '[OBJETO/BIEN]', !this.form.object_type);
                el('preview_object_description', this.form.object_description || this.form.object_type || '[DESCRIPCIÓN DEL OBJETO]', !this.form.object_description);
                ...
                el('preview_price', this.form.price_amount ? priceFormatted : '[PRECIO EN EUR]', !this.form.price_amount);
                ...
                el('preview_delivery_terms', this.form.delivery_terms ? `La entrega o formalización se efectuará conforme a lo siguiente: ${this.form.delivery_terms}` : 'La entrega del bien o formalización se efectuará en el plazo pactado.', !this.form.delivery_terms);
                el('preview_warranties', this.form.warranties || 'El bien se transmite libre de cargas no declaradas y con garantía conforme a la ley aplicable.', !this.form.warranties);
                check(this.form.payment_terms);

                // Type-specific clauses in preview
                const typeClauses = {
                    'arras': {
    ...
    function computeRegime() {
        ...
        if (notes) {
            ...
            const notesByRegime = {
                ...
                'b2c': 'IVA incluido en el precio; garantía legal y derecho de desistimiento (RDL 1/2007). En ventas a distancia a la UE puede aplicar OSS.',
                'c2c': 'Sin IVA; sujeta a ITP (RDL 1/1993) que abona el adquirente. En compraventas de inmuebles y arras aplica la normativa civil y registral.',
                'c2b': 'Sin IVA; posible retención IRPF 3% (art. 100 RIRPF) y régimen REBU del comprador.',
            };
            const key = txn + '-' + jur;
    ...
    // Listen to party input changes to update live preview
    document.querySelectorAll('input, select, textarea').forEach(input => {
        input.addEventListener('input', () => {
            const wizardEl = document.querySelector('[x-data="contractWizard()"]');
            if (wizardEl && wizardEl.__x) {
